<template>
  <div class="mod-user">
    ...
    <el-table
      ...
      <el-table-column prop="docNum" header-align="center" align="center" label="医护人数"/>
      <el-table-column prop="residualNum" header-align="center" align="center" label="剩余可预约人数"/>
      ...
      <el-table-column prop="status" header-align="center" align="center" label="状态">
        <template slot-scope="scope">
          ...
          <el-tag v-if="scope.row.status === 1" size="small">已启用</el-tag>
        </template>
      </el-table-column>
      <el-table-column fixed="right" header-align="center" align="center" width="150" label="操作">
        <template slot-scope="scope">
          <el-button type="text" size="small" @click="edit(scope.row.deptId)">修改</el-button>
...
<script>
import infoTemp from './ksgl-temp'
export default {
  components: {
    infoTemp
  },
  ...
  methods: {
    ...
    getDataList () {
      ...
      this.$http({
        url: this.$http.adornUrl('/sys/ksgl/list'),
        method: 'get',
    ...
    // 删除
    deleteHandle (id) {
      var deptIds = id ? [id] : this.dataListSelections.map(item => {
        return item.deptId
      })
      this.$confirm(`确定对该科室信息进行[${id ? '删除' : '批量删除'}]操作?`, '提示', {
        ...
        this.$http({
          url: this.$http.adornUrl('/sys/ksgl/delete'),
          method: 'post',
