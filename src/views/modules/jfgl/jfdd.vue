<template>
  <div class="mod-user">
    <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()">
      <el-form-item>
        <el-input v-model="dataForm.orderId" placeholder="订单编号" clearable @change="getDataList"></el-input>
      </el-form-item>
      <el-form-item>
      ...
      <el-select v-model="dataForm.status" placeholder="是否付款" clearable @change="getDataList">
    ...
    <el-table
      ...
      <el-table-column prop="orderId" header-align="center" align="center" width="80" label="订单编号"/>
      <el-table-column prop="patientId" header-align="center" align="center" label="患者编号"/>
      <el-table-column prop="prescriptionId" header-align="center" align="center" label="药物单编号"/>
      <el-table-column prop="orderTime" header-align="center" align="center" label="生成订单时间"/>
      <el-table-column prop="createUserId" header-align="center" align="center" label="创建人编号"/>
      <el-table-column prop="createTime" header-align="center" align="center" label="创建时间"/>
      <el-table-column prop="sum" header-align="center" align="center" width="100" label="总价">
        <template slot-scope="scope">
          <el-tag size="small">{{scope.row.sum}}</el-tag>
        </template>
      </el-table-column>
      <el-table-column prop="status" header-align="center" align="center" width="100" label="是否付款">
        <template slot-scope="scope">
          <el-tag v-if="scope.row.status === '0'" size="small" type="danger">未付款</el-tag>
          <el-tag v-else size="small">已付款</el-tag>
        </template>
      </el-table-column>
      <el-table-column fixed="right" header-align="center" align="center" width="180" label="操作">
        <template slot-scope="scope">
          <el-button type="text" size="small" @click="edit(scope.row.orderId)">详细</el-button>
...
<script>
import infoTemp from './jfdd-temp'
export default {
  components: {
    infoTemp
  },
  ...
  methods: {
    ...
    getDataList () {
      ...
      this.$http({
        url: this.$http.adornUrl('/sys/jfgl/list'),
        method: 'get',
    ...
    deleteHandle (id) {
      ...
      this.$confirm(`确定对该科室信息进行[${id ? '删除' : '批量删除'}]操作?`, '提示', {
        ...
        this.$http({
          url: this.$http.adornUrl('/sys/jfgl/delete'),
          method: 'post',
